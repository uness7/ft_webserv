...
Config::extractUnknownValue(const std::string &line) {
...
void Config::parseLocationLine(const std::string &line,
  ...
  if (line.find("root") != std::string::npos) {
    ...
    locationConfig.client_max_body_size =
        convertToBytes(locationConfig.content["client_max_body_size"]);
  } else if (line.find(' ') != std::string::npos) {
    std::map<std::string, std::string> content = extractUnknownValue(line);
    std::map<std::string, std::string>::iterator kv = content.begin();
...
void Config::printServerConfig(const ServerConfig &serverConfig) {
  ...
  if (serverConfig.client_max_body_size > 0)
    std::cout << "Client max body size: " << serverConfig.client_max_body_size
              << std::endl
              << std::endl;
...
void Config::printLocationConfig(const std::string &locationPath,
  ...
  if (!locationConfig.limit_except.empty()) {
    ...
    for (std::vector<std::string>::const_iterator method_it =
         ...
         method_it != locationConfig.limit_except.end(); ++method_it)
      std::cout << *method_it << " ";
    std::cout << std::endl;
  }
  ...
  std::map<std::string, std::string>::const_iterator f;
  for (f = locationConfig.content.begin(); f != locationConfig.content.end();
       f++)
    std::cout << f->first << " " << f->second << std::endl;
