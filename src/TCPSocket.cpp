...
TCPSocket::TCPSocket(const ServerConfig &serverConfig)
    : _serverConfig(serverConfig), _socketFD(-1),
      _ipAddress(serverConfig.listen), _port(serverConfig.port),
      _socketAddress(), _socketAddressLength(0) {
  ...
  _socketAddressLength = _socketAddress->ai_addrlen;
}
...
void TCPSocket::initSocket() {
  _socketFD = socket(_socketAddress->ai_family, _socketAddress->ai_socktype,
  ...
  if (bind(_socketFD, _socketAddress->ai_addr, _socketAddressLength) < 0)
    throw SocketInitException("Impossible to bind socket to address ",
                              getSocketAddressToString());
}

std::vector<TCPSocket *>
createSockets(const std::vector<ServerConfig> &serverConfigs) {
