...
bool Request::areHeadersValid() {
	...
	if (_target.allowed_methods.size() &&
	    std::find(_target.allowed_methods.begin(),
		      _target.allowed_methods.end(),
...
long Request::read(unsigned int fd) {
	...
	if (areHeadersValid() == false) return request.size();

	if (this->getMethod() == "POST") {
		readBody(fd, request, req_index);
		if (_statusCode) return request.size();
		if (this->getHeaderField("transfer-encoding") == "chunked") {
			bool isChunk = false;

			while (req_index < request.size()) {
				Bytes binaryLine = seekCRLF(request, req_index);
				if (binaryLine.empty()) break;
				if (isChunk)
			...
			_body =
			    Bytes(request.begin() + req_index, request.end());
	}
	return request.size();
}
...
void Request::readBody(unsigned int fd, Bytes &request, size_t index) {
	...
	if (getHeaderField("expect") == "100-continue" &&
		...
		while (true) {
			...
			std::string endRequest = std::string(
			    request.end() - end.size(), request.end());

			if (endRequest == end) break;
		}
	}
}
...
void Request::setMimeType() {
	if (_path.length() >= 5 &&
	    _path.substr(_path.length() - 5) == ".html") {
		_mimetype = "text/html";
		...
		_mimetype = "image/jpg";
		   ...
		   _path.substr(_path.length() - 3) == ".py") {
		_mimetype = "application/python";
	} else if (_path.length() >= 3 &&
		   _path.substr(_path.length() - 3) == ".js") {
