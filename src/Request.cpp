...
Request::Request()
    : _method(""), _path(""), _query(""), _mimetype(""), _body(), _headers(),
      _contentLength(0), _state(EMPTY) {
  /* Default Constructor */
}

Request::~Request() { /* Deconstructor */ }
...
bool Request::isValidClientMaxBody(ServerConfig &config,
                                   LocationConfig target) const {
  if (target.client_max_body_size > 0)
    return target.client_max_body_size > getContentLength();
  else if (config.client_max_body_size > 0)
    return config.client_max_body_size > getContentLength();
  return true;
}
...
bool Request::checkHeaderLocation(ServerConfig &config) {
  ...
  if (found < 0 || isValidClientMaxBody(config, target) == false)
    return false;
  ...
  if (allowed_methods.size() &&
      std::find(allowed_methods.begin(), allowed_methods.end(), getMethod()) ==
          allowed_methods.end())
    return false;
  return true;
}
...
long Request::readFromSocket(unsigned int socketFd, ServerConfig &config) {
  ...
  while (static_cast<long long>(totalRead) < _contentLength) {
    ...
    if (chunk.size() == 0)
      break;
    std::copy(chunk.begin(), chunk.begin() + chunk.size(),
              _body.begin() + totalRead);
