...
void Response::build(void) {
	...
	if (request.getStatusCode() != 0) {
		...
		updateResponse(_statusCode.code, "text/plain",
			       _statusCode.status);
...
std::vector<std::string> splitString(const std::string &str) {
	...
	if (std::getline(iss, part, ' ')) {
		...
		if (std::getline(iss, part)) result.push_back(part);
	}
	return result;
}
...
void Response::buildError() {
	...
	if (error.empty()) {
		setStatusCode(404);
		updateResponse(_statusCode.code, _statusCode.status,
			       _statusCode.status);
		return;
	}
...
void Response::handleCGI() {
	CGIResponse cgi =
	    CGIResponse(this->_client, _target.content["cgi_path"]);
	...
	if ((dir = opendir(directoryPath.c_str())) != NULL) {
		while ((entry = readdir(dir)) != NULL) {
			...
			if (S_ISDIR(fileStat.st_mode))
				...
				html << "<tr><td><a href=\"" << name;
			html << "\">" << name << "</a></td>" << "<td>"
			     << formatSize(fileStat.st_size) << "</td>"
			     << "<td>" << formatTime(fileStat.st_mtime)
			     << "</td></tr>\n";
		}
		closedir(dir);
	} else {
...
void Response::setStatusCode(unsigned short code) {
	const std::map<unsigned short, std::string> &statusCodes =
	    getStatusCodes();
